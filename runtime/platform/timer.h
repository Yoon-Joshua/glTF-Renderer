...
/// @brief Encapsulates basic usage of chrono, providing a means to calculate
/// float durations between time points via function calls.
class Timer {
 public:
  ...
  /// @brief Laps the timer, elapsed() now returns the duration since the last
  /// lap()
  void lap();
  ...
  /// @brief Calculates the time difference between now and when the timer was
  /// started if lap() was called, then between now and when the timer was last
  /// lapped
  ...
  /// @brief Calculates the time difference between now and the last time this
  /// function was called
  /// @return The duration between the two time points (default in seconds)
  template <typename T = DefaultResolution>
  double tick() {
