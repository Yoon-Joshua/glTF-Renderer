...
void mouse_button_callback(GLFWwindow* window, int button, int action,
  ...
  if (auto* platform =
    ...
    platform->input_event(MouseButtonInputEvent{
        translate_mouse_button(button), mouse_action, static_cast<float>(xpos),
        static_cast<float>(ypos)});
  }
}
...
Window::Window(Platform* platform, Window::Properties& properties) {
...
void Window::process_events() { glfwPollEvents(); }

void Window::close() { glfwSetWindowShouldClose(handle, GLFW_TRUE); }

GLFWwindow* Window::get_handle() const { return handle; }
...
const Window::Properties& Window::get_properties() const { return properties; }
