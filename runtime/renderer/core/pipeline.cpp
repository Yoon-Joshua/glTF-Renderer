...
VkPipeline Pipeline::get_handle() const { return handle; }

const PipelineState &Pipeline::get_state() const { return state; }

ComputePipeline::ComputePipeline(Device &device, VkPipelineCache pipeline_cache,
  ...
  if (shader_module->get_stage() != VK_SHADER_STAGE_COMPUTE_BIT) {
    throw VulkanException{VK_ERROR_INVALID_SHADER_NV,
                          "Shader module stage is not compute"};
  }
  ...
  vk_create_info.codeSize =
      shader_module->get_binary().size() * sizeof(uint8_t);
  vk_create_info.pCode =
      reinterpret_cast<const uint32_t *>(shader_module->get_binary().data());

  VkResult result = vkCreateShaderModule(device.get_handle(), &vk_create_info,
                                         nullptr, &stage.module);
  if (result != VK_SUCCESS) {
    throw VulkanException{result};
  }

  device.get_debug_utils().set_debug_name(
      device.get_handle(), VK_OBJECT_TYPE_SHADER_MODULE,
  ...
  for (const auto specialization_constant : specialization_constant_state) {
    map_entries.push_back({specialization_constant.first, to_u32(data.size()),
                           specialization_constant.second.size()});
    data.insert(data.end(), specialization_constant.second.begin(),
                specialization_constant.second.end());
  }
  ...
  result = vkCreateComputePipelines(device.get_handle(), pipeline_cache, 1,
...
GraphicsPipeline::GraphicsPipeline(Device &device,
  ...
  // Create specialization info from tracked state. This is shared by all
  // shaders.
  std::vector<uint8_t> data{};
  std::vector<VkSpecializationMapEntry> map_entries{};
  ...
  for (const ShaderModule *shader_module :
    ...
    VkResult result = vkCreateShaderModule(device.get_handle(), &vk_create_info,
                                           nullptr, &stage_create_info.module);
    if (result != VK_SUCCESS) {
      throw VulkanException{result};
    }
  ...
  auto result = vkCreateGraphicsPipelines(device.get_handle(), pipeline_cache,
