...
#include "runtime/renderer/fence_pool.h"
#include "runtime/renderer/resource_cache.h"
...
class Device : public core::VulkanResource<VkDevice, VK_OBJECT_TYPE_DEVICE> {
 public:
   ...
   * and whether support is optional or not
   */
  ...
  inline const DebugUtils& get_debug_utils() const { return *debug_utils; }

  /// @return Whether an image format is supported by the GPU
  bool is_image_format_supported(VkFormat format) const;

  const Queue& get_queue_by_flags(VkQueueFlags queue_flags,
  ...
  /// @return The first present supported queue, otherwise just any graphics
  /// queue
   ...
   * @param begin Whether the command buffer should be implicitly started before
   * it's returned
   ...
   * @param signalSemaphore An optional semaphore to signal when the commands
   * have been executed
   */
