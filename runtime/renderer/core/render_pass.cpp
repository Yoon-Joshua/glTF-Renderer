...
inline void set_pointer_next(
  ...
  // VkSubpassDescription cannot have pNext point to a
  // VkSubpassDescriptionDepthStencilResolveKHR containing a
...
inline const VkAttachmentReference2KHR *get_depth_resolve_reference(
...
inline const VkAttachmentReference2KHR *get_depth_resolve_reference(
  ...
  auto description_depth_resolve =
      static_cast<const VkSubpassDescriptionDepthStencilResolveKHR *>(
  ...
  if (description_depth_resolve) {
    depth_resolve_attachment =
        description_depth_resolve->pDepthStencilResolveAttachment;
  }

  return depth_resolve_attachment;
}

inline VkResult create_vk_renderpass(VkDevice device,
...
template <typename T>
std::vector<T> get_attachment_descriptions(
...
void set_attachment_layouts(
  ...
  for (auto &subpass : subpass_descriptions) {
    for (size_t k = 0U; k < subpass.colorAttachmentCount; ++k) {
      ...
      if (attachment_descriptions[reference.attachment].initialLayout ==
          VK_IMAGE_LAYOUT_UNDEFINED) {
        attachment_descriptions[reference.attachment].initialLayout =
            reference.layout;
      }
    }
    ...
    if (const auto depth_resolve = get_depth_resolve_reference(subpass)) {
      // Set it only if not defined yet
      if (attachment_descriptions[depth_resolve->attachment].initialLayout ==
          VK_IMAGE_LAYOUT_UNDEFINED) {
        attachment_descriptions[depth_resolve->attachment].initialLayout =
            depth_resolve->layout;
      }
    }
  }
  ...
  // Make the final layout same as the last subpass layout
  {
    ...
    for (size_t k = 0U; k < subpass.colorAttachmentCount; ++k) {
      ...
      attachment_descriptions[reference.attachment].finalLayout =
          reference.layout;
    }
    ...
    if (const auto depth_resolve = get_depth_resolve_reference(subpass)) {
      attachment_descriptions[depth_resolve->attachment].finalLayout =
          depth_resolve->layout;
    }
  }
}
...
template <typename T>
T get_attachment_reference(const uint32_t attachment,
...
void RenderPass::create_renderpass(
  ...
  auto attachment_descriptions =
      get_attachment_descriptions<T_AttachmentDescription>(attachments,
                                                           load_store_infos);

  // Store attachments for every subpass
  std::vector<std::vector<T_AttachmentReference>> input_attachments{
      subpass_count};
  std::vector<std::vector<T_AttachmentReference>> color_attachments{
      subpass_count};
  std::vector<std::vector<T_AttachmentReference>> depth_stencil_attachments{
      subpass_count};
  std::vector<std::vector<T_AttachmentReference>> color_resolve_attachments{
      subpass_count};
  std::vector<std::vector<T_AttachmentReference>> depth_resolve_attachments{
      subpass_count};
  ...
  if (needs_debug_name) {
    ...
    // subpasses.size());
    new_debug_name = "what a fuck, from \"render_pass.cpp\"";
  }
  ...
  for (size_t i = 0; i < subpasses.size(); ++i) {
    ...
    for (auto o_attachment : subpass.output_attachments) {
      auto initial_layout =
          attachments[o_attachment].initial_layout == VK_IMAGE_LAYOUT_UNDEFINED
              ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
      ...
      if (!is_depth_format(description.format)) {
        color_attachments[i].push_back(
            get_attachment_reference<T_AttachmentReference>(o_attachment,
                                                            initial_layout));
      }
    }
    ...
    for (auto i_attachment : subpass.input_attachments) {
      ...
      auto initial_layout =
          attachments[i_attachment].initial_layout == VK_IMAGE_LAYOUT_UNDEFINED
              ? default_layout
              : attachments[i_attachment].initial_layout;
      input_attachments[i].push_back(
          get_attachment_reference<T_AttachmentReference>(i_attachment,
                                                          initial_layout));
    }

    for (auto r_attachment : subpass.color_resolve_attachments) {
      auto initial_layout =
          attachments[r_attachment].initial_layout == VK_IMAGE_LAYOUT_UNDEFINED
              ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
              : attachments[r_attachment].initial_layout;
      color_resolve_attachments[i].push_back(
          get_attachment_reference<T_AttachmentReference>(r_attachment,
                                                          initial_layout));
    }

    if (!subpass.disable_depth_stencil_attachment) {
      // Assumption: depth stencil attachment appears in the list before any
      ...
      if (it != attachments.end()) {
        auto i_depth_stencil =
            vkb::to_u32(std::distance(attachments.begin(), it));
        auto initial_layout =
            it->initial_layout == VK_IMAGE_LAYOUT_UNDEFINED
                ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
                : it->initial_layout;
        depth_stencil_attachments[i].push_back(
            get_attachment_reference<T_AttachmentReference>(i_depth_stencil,
                                                            initial_layout));
        ...
        if (subpass.depth_stencil_resolve_mode != VK_RESOLVE_MODE_NONE) {
          ...
          depth_resolve_attachments[i].push_back(
              get_attachment_reference<T_AttachmentReference>(
                  i_depth_stencil_resolve, initial_layout));
        }
      }
    }
  }
  ...
  for (size_t i = 0; i < subpasses.size(); ++i) {
    ...
    subpass_description.inputAttachmentCount =
        to_u32(input_attachments[i].size());
    ...
    subpass_description.colorAttachmentCount =
        to_u32(color_attachments[i].size());
    ...
    if (!depth_stencil_attachments[i].empty()) {
      ...
      if (!depth_resolve_attachments[i].empty()) {
        ...
        // VkSubpassDescriptionDepthStencilResolve structure, then that
        // structure describes multisample resolve operations for the
        // depth/stencil attachment in a subpass
        depth_resolve.sType =
            VK_STRUCTURE_TYPE_SUBPASS_DESCRIPTION_DEPTH_STENCIL_RESOLVE_KHR;
        depth_resolve.depthResolveMode = subpass.depth_stencil_resolve_mode;
        set_pointer_next(subpass_description, depth_resolve,
  ...
  if (subpasses.empty()) {
    ...
    if (default_depth_stencil_attachment != VK_ATTACHMENT_UNUSED) {
      depth_stencil_attachments[0].push_back(
          get_attachment_reference<T_AttachmentReference>(
              default_depth_stencil_attachment,
              VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL));

      subpass_description.pDepthStencilAttachment =
          depth_stencil_attachments[0].data();
    }

    subpass_descriptions.push_back(subpass_description);
  }

  set_attachment_layouts<T_SubpassDescription, T_AttachmentDescription,
                         T_AttachmentReference>(subpass_descriptions,
  ...
  const auto &subpass_dependencies =
      get_subpass_dependencies<T_SubpassDependency>(subpass_count);
  ...
  auto result =
      create_vk_renderpass(device->get_handle(), create_info, &handle);
...
RenderPass::RenderPass(Device &device,
  ...
  if (device.is_enabled(VK_KHR_CREATE_RENDERPASS_2_EXTENSION_NAME)) {
    create_renderpass<VkSubpassDescription2KHR, VkAttachmentDescription2KHR,
                      VkAttachmentReference2KHR, VkSubpassDependency2KHR,
                      VkRenderPassCreateInfo2KHR>(attachments, load_store_infos,
                                                  subpasses);
  } else {
    create_renderpass<VkSubpassDescription, VkAttachmentDescription,
                      VkAttachmentReference, VkSubpassDependency,
                      VkRenderPassCreateInfo>(attachments, load_store_infos,
                                              subpasses);
  }
}
...
const uint32_t RenderPass::get_color_output_count(
...
const VkExtent2D RenderPass::get_render_area_granularity() const {
  ...
  vkGetRenderAreaGranularity(device->get_handle(), get_handle(),
                             &render_area_granularity);
