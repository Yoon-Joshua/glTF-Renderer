...
inline uint32_t choose_image_array_layers(uint32_t request_image_array_layers,
  ...
  request_image_array_layers =
      std::min(request_image_array_layers, max_image_array_layers);
...
inline VkExtent2D choose_extent(VkExtent2D request_extent,
  ...
  if (request_extent.width < 1 || request_extent.height < 1) {
    LOGW(
        "(Swapchain) Image extent (%d, %d) not supported. Selecting (%d, "
        "%d).\n",
        request_extent.width, request_extent.height, current_extent.width,
...
inline VkPresentModeKHR choose_present_mode(
  ...
  if (present_mode_it == available_present_modes.end()) {
    ...
    /* LOGI("(Swapchain) Present mode selected: {}",
...
inline VkSurfaceTransformFlagBitsKHR choose_transform(
  ...
  /* LOGW("(Swapchain) Surface transform '{}' not supported. Selecting '{}'.",
       to_string(request_transform), to_string(current_transform)); */
...
inline bool validate_format_feature(VkImageUsageFlagBits image_usage,
...
inline std::set<VkImageUsageFlagBits> choose_image_usage(
  ...
  for (auto flag : requested_image_usage_flags) {
    if ((flag & supported_image_usage) &&
        validate_format_feature(flag, supported_features)) {
      validated_image_usage_flags.insert(flag);
    } else {
      /* LOGW("(Swapchain) Image usage ({}) requested but not supported.",
           to_string(flag)); */
    }
  }
  ...
  if (!validated_image_usage_flags.empty()) {
    ...
    LOGI("(Swapchain) Image usage flags: %s\n", usage_list.c_str());
  } else {
...
inline VkImageUsageFlags composite_image_flags(
...
Swapchain::Swapchain(
    ...
    const std::vector<VkSurfaceFormatKHR>& surface_format_priority_list,
    const VkExtent2D& extent, const uint32_t image_count,
    const VkSurfaceTransformFlagBitsKHR transform,
    const std::set<VkImageUsageFlagBits>& image_usage_flags)
    : Swapchain{*this,
                device,
...
Swapchain::Swapchain(
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(this->device.get_gpu().get_handle(),
                                            surface, &surface_capabilities);

  uint32_t surface_format_count{0U};
  VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
      this->device.get_gpu().get_handle(), surface, &surface_format_count,
      nullptr));
  ...
  VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(
      this->device.get_gpu().get_handle(), surface, &present_mode_count,
      nullptr));
  ...
  properties.image_count =
      choose_image_count(image_count, surface_capabilities.minImageCount,
  ...
  properties.array_layers =
      choose_image_array_layers(1U, surface_capabilities.maxImageArrayLayers);
  properties.surface_format = choose_surface_format(
      properties.surface_format, surface_formats, surface_format_priority_list);
  VkFormatProperties format_properties;
  vkGetPhysicalDeviceFormatProperties(this->device.get_gpu().get_handle(),
                                      properties.surface_format.format,
                                      &format_properties);
  this->image_usage_flags = choose_image_usage(
      image_usage_flags, surface_capabilities.supportedUsageFlags,
  ...
  properties.composite_alpha =
      choose_composite_alpha(VK_COMPOSITE_ALPHA_INHERIT_BIT_KHR,
  ...
  properties.present_mode = choose_present_mode(
      properties.present_mode, present_modes, present_mode_priority_list);
  properties.surface_format = choose_surface_format(
      properties.surface_format, surface_formats, surface_format_priority_list);
  ...
  VkResult result =
      vkCreateSwapchainKHR(device.get_handle(), &create_info, nullptr, &handle);
  ...
  VK_CHECK(vkGetSwapchainImagesKHR(device.get_handle(), handle,
                                   &image_available, nullptr));

  images.resize(image_available);

  VK_CHECK(vkGetSwapchainImagesKHR(device.get_handle(), handle,
                                   &image_available, images.data()));
}
...
VkSwapchainKHR Swapchain::get_handle() const { return handle; }

VkResult Swapchain::acquire_next_image(uint32_t& image_index,
                                       VkSemaphore image_acquired_semaphore,
                                       VkFence fence) const {
  return vkAcquireNextImageKHR(device.get_handle(), handle,
                               std::numeric_limits<uint64_t>::max(),
                               image_acquired_semaphore, fence, &image_index);
}

const VkExtent2D& Swapchain::get_extent() const { return properties.extent; }
...
VkSurfaceKHR Swapchain::get_surface() const { return surface; }
