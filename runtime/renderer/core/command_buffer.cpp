...
#include "runtime/renderer/rendering/render_frame.h"
...
CommandBuffer::CommandBuffer(CommandPool &command_pool,
    ...
    : VulkanResource{VK_NULL_HANDLE, &command_pool.get_device()},
      ...
      max_push_constants_size{
          device->get_gpu().get_properties().limits.maxPushConstantsSize},
      level{level} {
  ...
  VkResult result =
      vkAllocateCommandBuffers(device->get_handle(), &allocate_info, &handle);
...
CommandBuffer::~CommandBuffer() {
  ...
  if (handle != VK_NULL_HANDLE) {
    vkFreeCommandBuffers(command_pool.get_device().get_handle(),
                         command_pool.get_handle(), 1, &handle);
  }
}
...
CommandBuffer::CommandBuffer(CommandBuffer &&other)
    : VulkanResource(std::move(other)),
      ...
      descriptor_set_layout_binding_state(
          std::exchange(other.descriptor_set_layout_binding_state, {})) {}
...
VkResult CommandBuffer::begin(VkCommandBufferUsageFlags flags,
                              CommandBuffer *primary_cmd_buf) {
  if (level == VK_COMMAND_BUFFER_LEVEL_SECONDARY) {
#ifdef TEMP
    printf(
        "currently unable to use VK_COMMAND_BUFFER_LEVEL_SECONDARY, "
        "\"command_buffer.cpp\", by XR.Y");
    exit(-1);
#else
    assert(primary_cmd_buf &&
           "A primary command buffer pointer must be provided when calling "
           "begin from a secondary one");
    auto render_pass_binding = primary_cmd_buf->get_current_render_pass();

    return begin(flags, render_pass_binding.render_pass,
                 render_pass_binding.framebuffer,
...
VkResult CommandBuffer::begin(VkCommandBufferUsageFlags flags,
  ...
  if (level == VK_COMMAND_BUFFER_LEVEL_SECONDARY) {
    ...
    assert((render_pass && framebuffer) &&
           "Render pass and framebuffer must be provided when calling begin "
...
void CommandBuffer::begin_render_pass(
  ...
  auto &render_pass =
      get_render_pass(render_target, load_store_infos, subpasses);
  auto &framebuffer = get_device().get_resource_cache().request_framebuffer(
      render_target, render_pass);

  begin_render_pass(render_target, render_pass, framebuffer, clear_values,
                    contents);
}
...
void CommandBuffer::begin_render_pass(
  ...
  const auto &framebuffer_extent =
      current_render_pass.framebuffer->get_extent();

  // Test the requested render area to confirm that it is optimal and could not
  // cause a performance reduction
  if (!is_render_size_optimal(framebuffer_extent, begin_info.renderArea)) {
    // Only prints the warning if the framebuffer or render area are different
    // since the last time the render size was not optimal
    if (framebuffer_extent.width != last_framebuffer_extent.width ||
      ...
      LOGW(
          "Render target extent is not an optimal size, this may result in "
          "reduced performance.");
    }
  ...
  blend_state.attachments.resize(
      current_render_pass.render_pass->get_color_output_count(
...
void CommandBuffer::end_render_pass() { vkCmdEndRenderPass(get_handle()); }

void CommandBuffer::bind_pipeline_layout(PipelineLayout &pipeline_layout) {
  pipeline_state.set_pipeline_layout(pipeline_layout);
}

void CommandBuffer::set_specialization_constant(
    uint32_t constant_id, const std::vector<uint8_t> &data) {
...
void CommandBuffer::push_constants(const std::vector<uint8_t> &values) {
  uint32_t push_constant_size =
      to_u32(stored_push_constants.size() + values.size());

  if (push_constant_size > max_push_constants_size) {
    LOGE(
        "Push constant limit of {} exceeded (pushing {} bytes for a total of "
        "{} bytes)",
...
void CommandBuffer::bind_buffer(const core::Buffer &buffer, VkDeviceSize offset,
                                VkDeviceSize range, uint32_t set,
                                uint32_t binding, uint32_t array_element) {
  resource_binding_state.bind_buffer(buffer, offset, range, set, binding,
                                     array_element);
}

void CommandBuffer::bind_image(const core::ImageView &image_view,
                               const core::Sampler &sampler, uint32_t set,
                               uint32_t binding, uint32_t array_element) {
  resource_binding_state.bind_image(image_view, sampler, set, binding,
                                    array_element);
}

void CommandBuffer::bind_image(const core::ImageView &image_view, uint32_t set,
                               uint32_t binding, uint32_t array_element) {
  resource_binding_state.bind_image(image_view, set, binding, array_element);
}

void CommandBuffer::bind_input(const core::ImageView &image_view, uint32_t set,
                               uint32_t binding, uint32_t array_element) {
...
void CommandBuffer::bind_vertex_buffers(
    ...
    const std::vector<std::reference_wrapper<const vkb::core::Buffer>> &buffers,
  ...
  vkCmdBindVertexBuffers(get_handle(), first_binding,
                         to_u32(buffer_handles.size()), buffer_handles.data(),
                         offsets.data());
}

void CommandBuffer::bind_index_buffer(const core::Buffer &buffer,
                                      VkDeviceSize offset,
                                      VkIndexType index_type) {
  vkCmdBindIndexBuffer(get_handle(), buffer.get_handle(), offset, index_type);
}

void CommandBuffer::bind_lighting(LightingState &lighting_state, uint32_t set,
                                  uint32_t binding) {
  ...
  set_specialization_constant(0,
                              to_u32(lighting_state.directional_lights.size()));
...
void CommandBuffer::set_rasterization_state(
    const RasterizationState &state_info) {
...
void CommandBuffer::set_viewport(uint32_t first_viewport,
  ...
  vkCmdSetViewport(get_handle(), first_viewport, to_u32(viewports.size()),
                   viewports.data());
}

void CommandBuffer::set_scissor(uint32_t first_scissor,
                                const std::vector<VkRect2D> &scissors) {
  vkCmdSetScissor(get_handle(), first_scissor, to_u32(scissors.size()),
                  scissors.data());
}
...
void CommandBuffer::draw(uint32_t vertex_count, uint32_t instance_count,
  ...
  vkCmdDraw(get_handle(), vertex_count, instance_count, first_vertex,
            first_instance);
}
...
void CommandBuffer::draw_indexed(uint32_t index_count, uint32_t instance_count,
  ...
  vkCmdDrawIndexed(get_handle(), index_count, instance_count, first_index,
...
void CommandBuffer::copy_buffer_to_image(
  ...
  vkCmdCopyBufferToImage(get_handle(), buffer.get_handle(), image.get_handle(),
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                         to_u32(regions.size()), regions.data());
}

void CommandBuffer::image_memory_barrier(
  ...
  vkb::image_layout_transition(
      get_handle(), image_view.get_image().get_handle(),
      memory_barrier.src_stage_mask, memory_barrier.dst_stage_mask,
      memory_barrier.src_access_mask, memory_barrier.dst_access_mask,
      memory_barrier.old_layout, memory_barrier.new_layout, subresource_range);
}
...
VkResult CommandBuffer::reset(ResetMode reset_mode) {
  ...
  assert(reset_mode == command_pool.get_reset_mode() &&
         "Command buffer reset mode must match the one used by the pool to "
         "allocate it");
...
RenderPass &CommandBuffer::get_render_pass(
  ...
  for (auto &subpass : subpasses) {
    ...
    subpass_info_it->color_resolve_attachments =
        subpass->get_color_resolve_attachments();
    ...
    subpass_info_it->depth_stencil_resolve_mode =
        subpass->get_depth_stencil_resolve_mode();
  ...
  return get_device().get_resource_cache().request_render_pass(
      render_target.get_attachments(), load_store_infos, subpass_infos);
}

const bool CommandBuffer::is_render_size_optimal(
    const VkExtent2D &framebuffer_extent, const VkRect2D &render_area) {
  auto render_area_granularity =
      current_render_pass.render_pass->get_render_area_granularity();
...
void CommandBuffer::flush_pipeline_state(
  ...
  if (pipeline_bind_point == VK_PIPELINE_BIND_POINT_GRAPHICS) {
    ...
    auto &pipeline =
        get_device().get_resource_cache().request_graphics_pipeline(
            pipeline_state);

    vkCmdBindPipeline(get_handle(), pipeline_bind_point, pipeline.get_handle());
  } else if (pipeline_bind_point == VK_PIPELINE_BIND_POINT_COMPUTE) {
    auto &pipeline = get_device().get_resource_cache().request_compute_pipeline(
        pipeline_state);
...
void CommandBuffer::flush_descriptor_state(
    VkPipelineBindPoint pipeline_bind_point) {
  assert(command_pool.get_render_frame() &&
  ...
  if (resource_binding_state.is_dirty() || !update_descriptor_sets.empty()) {
    ...
    for (auto &resource_set_it : resource_binding_state.get_resource_sets()) {
      ...
      // Don't update resource set if it's not in the update list OR its state
      // hasn't changed
      if (!resource_set.is_dirty() &&
          (update_descriptor_sets.find(descriptor_set_id) ==
      ...
      descriptor_set_layout_binding_state[descriptor_set_id] =
        ...
        if (auto binding_info =
          ...
          for (auto &element_it : binding_resources) {
            ...
            if (buffer != nullptr &&
              ...
              if (is_dynamic_buffer_descriptor_type(
            ...
            // Get image info
              ...
              image_info.sampler =
                  sampler ? sampler->get_handle() : VK_NULL_HANDLE;
          ...
          assert((!update_after_bind ||
                  (buffer_infos.count(binding_index) > 0 ||
                   (image_infos.count(binding_index) > 0))) &&
                 "binding index with no buffer or image infos can't be checked "
                 "for adding to bindings_to_update");
        }
      }

      VkDescriptorSet descriptor_set_handle =
          command_pool.get_render_frame()->request_descriptor_set(
              descriptor_set_layout, buffer_infos, image_infos,
              update_after_bind, command_pool.get_thread_index());

      // Bind descriptor set
      vkCmdBindDescriptorSets(
          get_handle(), pipeline_bind_point, pipeline_layout.get_handle(),
...
void CommandBuffer::flush_push_constants() {
  ...
  VkShaderStageFlags shader_stage =
      pipeline_layout.get_push_constant_range_stage(
          to_u32(stored_push_constants.size()));

  if (shader_stage) {
    vkCmdPushConstants(get_handle(), pipeline_layout.get_handle(), shader_stage,
                       0, to_u32(stored_push_constants.size()),
                       stored_push_constants.data());
  } else {
    LOGW("Push constant range [{}, {}] not found", 0,
         stored_push_constants.size());
  }
...
template <class T>
inline void CommandBuffer::set_specialization_constant(uint32_t constant_id,
                                                       const T &data) {
  set_specialization_constant(constant_id, to_bytes(data));
}

template <>
inline void CommandBuffer::set_specialization_constant<bool>(
    std::uint32_t constant_id, const bool &data) {
