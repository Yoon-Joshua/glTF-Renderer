...
/// @brief Adds support for C style preprocessor macros to glsl shaders enabling
/// you to define or undefine certain symbols
class ShaderVariant {
 public:
  ...
  /// @brief Specifies the size of a named runtime array for automatic
  /// reflection. If already specified, overrides the size.
  /// @param runtime_array_name String under which the runtime array is named in
  /// the shader
  /// @param size Integer specifying the wanted size of the runtime array (in
  /// number of elements, not size in bytes), used for automatic allocation of
  /// buffers. See get_declared_struct_size_runtime_array() in spirv_cross.h
  void add_runtime_array_size(const std::string &runtime_array_name,
                              size_t size);

  void set_runtime_array_sizes(
  ...
  const std::unordered_map<std::string, size_t> &get_runtime_array_sizes()
      const;
...
/// @brief Contains shader code, with an entry point, for a specific shader
/// stage. It is needed by a PipelineLayout to create a Pipeline. ShaderModule
/// can do auto-pairing between shader code and textures. The low level code can
/// change bindings, just keeping the name of the texture. Variants for each
/// texture are also generated, such as HAS_BASE_COLOR_TEX. It works similarly
/// for attribute locations. A current limitation is that only set 0 is
/// considered. Uniform buffers are currently hardcoded as well.
class ShaderModule {
  /************************** XR.Y **************************/
  // 配置glslang和spirv-cross太难了，我也不想在线编译。
 public:
 ...
 public:
  ShaderModule(Device &device, VkShaderStageFlagBits stage,
               const ShaderSource &glsl_source, const std::string &entry_point,
               const ShaderVariant &shader_variant);
  ...
  inline void set_debug_name(const std::string &name) { debug_name = name; }

  /// @brief Flags a resource to use a different method of being bound to the
  /// shader
  /// @param resource_name The name of the shader resource
  /// @param resource_mode The mode of how the shader resource will be bound
  void set_resource_mode(const std::string &resource_name,
