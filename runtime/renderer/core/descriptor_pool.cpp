...
void DescriptorPool::set_descriptor_set_layout(
...
VkDescriptorSet DescriptorPool::allocate() {
  ...
  auto result =
      vkAllocateDescriptorSets(device.get_handle(), &alloc_info, &handle);
...
VkResult DescriptorPool::free(VkDescriptorSet descriptor_set) {
  ...
  vkFreeDescriptorSets(device.get_handle(), pools[desc_pool_index], 1,
                       &descriptor_set);
...
std::uint32_t DescriptorPool::find_available_pool(std::uint32_t search_index) {
  ...
  if (pools.size() <= search_index) {
    ...
    // We do not set FREE_DESCRIPTOR_SET_BIT as we do not need to free
    // individual descriptor sets
    create_info.flags = 0;
    ...
    auto result = vkCreateDescriptorPool(device.get_handle(), &create_info,
                                         nullptr, &handle);
