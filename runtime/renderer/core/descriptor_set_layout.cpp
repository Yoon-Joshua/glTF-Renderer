...
DescriptorSetLayout::DescriptorSetLayout(
    Device &device, const uint32_t set_index,
    const std::vector<ShaderModule *> &shader_modules,
    const std::vector<ShaderResource> &resource_set)
    : device{device}, set_index{set_index}, shader_modules{shader_modules} {
  // NOTE: `shader_modules` is passed in mainly for hashing their handles in
  // `request_resource`.
  //        This way, different pipelines (with different shaders / shader
  //        variants) will get different descriptor set layouts (incl.
  //        appropriate name -> binding lookups)
  ...
  for (auto &resource : resource_set) {
    ...
    if (resource.mode == ShaderResourceMode::UpdateAfterBind) {
      ...
      // When creating a descriptor set layout, if we give a structure to
      // create_info.pNext, each binding needs to have a binding flag
      // (pBindings[i] uses the flags in pBindingFlags[i])
      // Adding 0 ensures the bindings that dont use any flags are mapped
      // correctly.
  ...
  if (std::find_if(resource_set.begin(), resource_set.end(),
    ...
    if (std::find_if(resource_set.begin(), resource_set.end(),
      ...
      throw std::runtime_error(
          "Cannot create descriptor set layout, dynamic resources are not "
          "allowed if at least one resource is update-after-bind.");
    }

    if (!validate_flags(device.get_gpu(), bindings, binding_flags)) {
      throw std::runtime_error(
          "Invalid binding, couldn't create descriptor set layout.");
    }
    ...
    create_info.flags |=
        std::find(binding_flags.begin(), binding_flags.end(),
  ...
  VkResult result = vkCreateDescriptorSetLayout(device.get_handle(),
...
VkDescriptorSetLayout DescriptorSetLayout::get_handle() const { return handle; }

const uint32_t DescriptorSetLayout::get_index() const { return set_index; }

const std::vector<VkDescriptorSetLayoutBinding> &
DescriptorSetLayout::get_bindings() const {
  return bindings;
}

std::unique_ptr<VkDescriptorSetLayoutBinding>
DescriptorSetLayout::get_layout_binding(const uint32_t binding_index) const {
...
const std::vector<VkDescriptorBindingFlagsEXT> &
DescriptorSetLayout::get_binding_flags() const {
  return binding_flags;
}

VkDescriptorBindingFlagsEXT DescriptorSetLayout::get_layout_binding_flag(
