...
ImageView::ImageView(Image& img, VkImageViewType view_type, VkFormat format,
  ...
  subresource_range.layerCount = n_array_layers == 0
                                     ? image->get_subresource().arrayLayer
                                     : n_array_layers;
  ...
  auto result =
      vkCreateImageView(device->get_handle(), &view_info, nullptr, &handle);
...
void ImageView::set_image(Image& img) { image = &img; }

VkFormat ImageView::get_format() const { return format; }
