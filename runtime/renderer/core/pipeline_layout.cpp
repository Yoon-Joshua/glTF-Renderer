...
PipelineLayout::PipelineLayout(
  ...
  // Collect and combine all the shader resources from each of the shader
  // modules Collate them all into a map that is indexed by the name of the
  // resource
  for (auto *shader_module : shader_modules) {
    for (const auto &shader_resource : shader_module->get_resources()) {
      std::string key = shader_resource.name;

      // Since 'Input' and 'Output' resources can have the same name, we modify
      // the key string
  ...
  for (auto &it : shader_resources) {
    ...
    if (it2 != shader_sets.end()) {
      ...
      shader_sets.emplace(shader_resource.set,
  ...
  for (auto &shader_set_it : shader_sets) {
    descriptor_set_layouts.emplace_back(
        &device.get_resource_cache().request_descriptor_set_layout(
  ...
  for (uint32_t i = 0; i < descriptor_set_layouts.size(); ++i) {
    if (descriptor_set_layouts[i]) {
      descriptor_set_layout_handles.push_back(
          descriptor_set_layouts[i]->get_handle());
    } else {
  ...
  for (auto &push_constant_resource :
       get_resources(ShaderResourceType::PushConstant)) {
  ...
  auto result = vkCreatePipelineLayout(device.get_handle(), &create_info,
                                       nullptr, &handle);
...
VkPipelineLayout PipelineLayout::get_handle() const { return handle; }

const std::vector<ShaderModule *> &PipelineLayout::get_shader_modules() const {
  return shader_modules;
}

const std::vector<ShaderResource> PipelineLayout::get_resources(
    const ShaderResourceType &type, VkShaderStageFlagBits stage) const {
...
PipelineLayout::get_shader_sets() const {
  return shader_sets;
}
...
DescriptorSetLayout &PipelineLayout::get_descriptor_set_layout(
  ...
  throw std::runtime_error("Couldn't find descriptor set layout at set index " +
                           to_string(set_index));
}

VkShaderStageFlags PipelineLayout::get_push_constant_range_stage(
