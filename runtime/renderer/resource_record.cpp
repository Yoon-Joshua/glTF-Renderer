...
namespace {
inline void write_subpass_info(std::ostringstream &os,
...
inline void write_processes(std::ostringstream &os,
...
const std::ostringstream &ResourceRecord::get_stream() { return stream; }
...
size_t ResourceRecord::register_shader_module(
  ...
  write(stream, ResourceType::ShaderModule, stage, glsl_source.get_source(),
        entry_point, shader_variant.get_preamble());
...
size_t ResourceRecord::register_pipeline_layout(
  ...
  std::transform(shader_modules.begin(), shader_modules.end(),
                 shader_indices.begin(), [this](ShaderModule *shader_module) {
...
size_t ResourceRecord::register_render_pass(
...
size_t ResourceRecord::register_graphics_pipeline(
  ...
  write(stream, ResourceType::GraphicsPipeline,
  ...
  write(stream, pipeline_state.get_input_assembly_state(),
  ...
  write(stream, color_blend_state.logic_op, color_blend_state.logic_op_enable,
        color_blend_state.attachments);

  return graphics_pipeline_indices.back();
}

void ResourceRecord::set_shader_module(size_t index,
                                       const ShaderModule &shader_module) {
  shader_module_to_index[&shader_module] = index;
}

void ResourceRecord::set_pipeline_layout(
    size_t index, const PipelineLayout &pipeline_layout) {
  pipeline_layout_to_index[&pipeline_layout] = index;
}

void ResourceRecord::set_render_pass(size_t index,
                                     const RenderPass &render_pass) {
  render_pass_to_index[&render_pass] = index;
}

void ResourceRecord::set_graphics_pipeline(
    size_t index, const GraphicsPipeline &graphics_pipeline) {
