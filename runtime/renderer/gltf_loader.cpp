...
inline std::vector<uint8_t> convert_underlying_data_stride(
    const std::vector<uint8_t> &src_data, uint32_t src_stride,
    uint32_t dst_stride) {
...
inline void upload_image_to_gpu(CommandBuffer &command_buffer,
                                core::Buffer &staging_buffer,
                                sg::Image &image) {
...

  {
    ...
    command_buffer.image_memory_barrier(image.get_vk_image_view(),
                                        memory_barrier);
  }
  ...
  for (size_t i = 0; i < mipmaps.size(); ++i) {
    ...
    copy_region.imageSubresource =
        image.get_vk_image_view().get_subresource_layers();
    // Update miplevel
    copy_region.imageSubresource.mipLevel = mipmap.level;
    copy_region.imageExtent = mipmap.extent;
  }

  command_buffer.copy_buffer_to_image(staging_buffer, image.get_vk_image(),
                                      buffer_copy_regions);
...
static inline bool texture_needs_srgb_colorspace(const std::string &name) {
  // The gltf spec states that the base and emissive textures MUST be encoded
  // with the sRGB transfer function. All other texture types are linear.
  if (name == "baseColorTexture" || name == "emissiveTexture") {
    return true;
  }

  // metallicRoughnessTexture, normalTexture & occlusionTexture must be linear
  assert(name == "metallicRoughnessTexture" || name == "normalTexture" ||
...
GLTFLoader::GLTFLoader(Device const &device) : device{device} {}

std::unique_ptr<sg::Scene> GLTFLoader::read_scene_from_file(
    const std::string &file_name, int scene_index) {
  std::string err;
  std::string warn;
  ...
  bool importResult =
      gltf_loader.LoadASCIIFromFile(&model, &err, &warn, gltf_file.c_str());
  ...
  if (!err.empty()) {
    LOGE("Error loading gltf model: %s.\n", err.c_str());
    return nullptr;
  }
...
std::unique_ptr<sg::Node> GLTFLoader::parse_node(
    const tinygltf::Node &gltf_node, size_t index) const {
...
std::unique_ptr<sg::Mesh> GLTFLoader::parse_mesh(
    const tinygltf::Mesh &gltf_mesh) const {
...
std::unique_ptr<sg::PBRMaterial> GLTFLoader::parse_material(
  ...
  for (auto &gltf_value : gltf_material.values) {
    if (gltf_value.first == "baseColorFactor") {
      ...
      material->base_color_factor = glm::vec4(color_factor[0], color_factor[1],
                                              color_factor[2], color_factor[3]);
    } else if (gltf_value.first == "metallicFactor") {
      material->metallic_factor =
          static_cast<float>(gltf_value.second.Factor());
  ...
  for (auto &gltf_value : gltf_material.additionalValues) {
    if (gltf_value.first == "emissiveFactor") {
      ...
      material->alpha_cutoff =
          static_cast<float>(gltf_value.second.number_value);
...
std::unique_ptr<sg::Image> GLTFLoader::parse_image(
    tinygltf::Image &gltf_image) const {
  ...
  if (!gltf_image.image.empty()) {
    ...
    image = std::make_unique<sg::Image>(
        gltf_image.name, std::move(gltf_image.image), std::move(mipmaps));
  } else {
    // Load image from uri
  ...
  if (sg::is_astc(image->get_format())) {
    ...
    //   LOGW("ASTC not supported: decoding %s\n", image->get_name().c_str());
    //   image = std::make_unique<sg::Astc>(*image);
    //   image->generate_mipmaps();
    // }
    printf("Don't support astc!\n");
    std::exit(-1);
  }
...
std::unique_ptr<sg::Sampler> GLTFLoader::parse_sampler(
  ...
  VkSamplerAddressMode address_mode_w = VK_SAMPLER_ADDRESS_MODE_MIRRORED_REPEAT;
...
std::vector<std::unique_ptr<sg::Light>>
GLTFLoader::parse_khr_lights_punctual() {
  if (is_extension_enabled(KHR_LIGHTS_PUNCTUAL_EXTENSION)) {
    ...
    for (size_t light_index = 0; light_index < khr_lights.ArrayLen();
         ++light_index) {
      ...
      if (!khr_light.Has("type")) {
        LOGE("KHR_lights_punctual extension: light {} doesn't have a type!",
             light_index);
        throw std::runtime_error(
            "Couldn't load glTF file, KHR_lights_punctual extension is "
            "invalid");
      }

      auto light =
          std::make_unique<sg::Light>(khr_light.Get("name").Get<std::string>());
      ...
      if (gltf_light_type == "point") {
        ...
        LOGE("KHR_lights_punctual extension: light type '{}' is invalid\n",
             gltf_light_type);
        throw std::runtime_error(
            "Couldn't load glTF file, KHR_lights_punctual extension is "
            "invalid\n");
      ...
      if (type != sg::LightType::Directional) {
        ...
        if (type != sg::LightType::Point) {
          if (!khr_light.Has("spot")) {
            LOGE(
                "KHR_lights_punctual extension: spot light doesn't have a "
                "'spot' property set",
                gltf_light_type);
            throw std::runtime_error(
                "Couldn't load glTF file, KHR_lights_punctual extension is "
          ...
          properties.inner_cone_angle = static_cast<float>(
              khr_light.Get("spot").Get("innerConeAngle").Get<double>());

          if (khr_light.Get("spot").Has("outerConeAngle")) {
            properties.outer_cone_angle = static_cast<float>(
      ...
      } else if (type == sg::LightType::Directional ||
                 type == sg::LightType::Spot) {
        // The spec states that the light will inherit the transform of the
        // node. The light's direction is defined as the 3-vector (0.0, 0.0,
        // -1.0) and the rotation of the node orients the light accordingly.
...
tinygltf::Value *GLTFLoader::get_extension(
    tinygltf::ExtensionMap &tinygltf_extensions, const std::string &extension) {
...
sg::Scene GLTFLoader::load_scene(int scene_index) {
  ...
  for (auto &used_extension : model.extensionsUsed) {
    ...
    if (it == supported_extensions.end()) {
      ...
      if (std::find(model.extensionsRequired.begin(),
                    ...
                    used_extension) != model.extensionsRequired.end()) {
        throw std::runtime_error(
            "Cannot load glTF file. Contains a required unsupported "
            "extension: " +
            used_extension);
      } else {
        // Otherwise, if extension isn't required (but is in the file) then
        // print a warning to the user
        LOGW(
            "glTF file contains an unsupported extension, unexpected results "
            "may occur: {}\n",
            used_extension);
      }
    } else {
      // Extension is supported, so enable it
      LOGI("glTF file contains extension: {}\n", used_extension);
      it->second = true;
    }
  }
  ...
  LOGI("Time spent loading images: {} seconds across {} threads.",
       vkb::to_string(elapsed_time), thread_count);
#endif
  ...
  for (auto &gltf_texture : model.textures) {
    ...
    if (gltf_texture.sampler >= 0 &&
        gltf_texture.sampler < static_cast<int>(samplers.size())) {
  ...
  for (auto &gltf_mesh : model.meshes) {
    ...
    for (size_t i_primitive = 0; i_primitive < gltf_mesh.primitives.size();
      ...
      for (auto &attribute : gltf_primitive.attributes) {
        ...
        std::transform(attrib_name.begin(), attrib_name.end(),
                       attrib_name.begin(), ::tolower);
        ...
        if (attrib_name == "position") {
          ...
          submesh->vertices_count =
              to_u32(model.accessors[attribute.second].count);
        }
      ...
      if (gltf_primitive.indices >= 0) {
        ...
        switch (format) {
          case VK_FORMAT_R8_UINT:
            // Converts uint8 data into uint16 data, still represented by a
            // uint8 vector
        ...
        submesh->vertices_count = to_u32(get_attribute_size(
            &model, gltf_primitive.attributes.at("POSITION")));
      }
  ...
  for (size_t node_index = 0; node_index < model.nodes.size(); ++node_index) {
    ...
    if (auto extension = get_extension(gltf_node.extensions,
  ...
  // Load animations
  for (size_t animation_index = 0; animation_index < model.animations.size();
       ++animation_index) {
    auto &gltf_animation = model.animations[animation_index];

    std::vector<sg::AnimationSampler> samplers;

    for (size_t sampler_index = 0;
         sampler_index < gltf_animation.samplers.size(); ++sampler_index) {
      ...
      if (gltf_sampler.interpolation == "LINEAR") {
        ...
        LOGW("Gltf animation sampler #{} has unknown interpolation value",
             sampler_index);
      }
      ...
      const float *data =
          reinterpret_cast<const float *>(input_accessor_data.data());
      ...
      auto output_accessor_data =
          get_attribute_data(&model, gltf_sampler.output);

      switch (output_accessor.type) {
        case TINYGLTF_TYPE_VEC3: {
          const glm::vec3 *data =
              reinterpret_cast<const glm::vec3 *>(output_accessor_data.data());
        ...
        case TINYGLTF_TYPE_VEC4: {
          const glm::vec4 *data =
              reinterpret_cast<const glm::vec4 *>(output_accessor_data.data());
          for (size_t i = 0; i < output_accessor.count; ++i) {
        ...
        default: {
          LOGW("Gltf animation sampler #{} has unknown output data type",
               sampler_index);
          continue;
        }
      }
    ...
    for (size_t channel_index = 0;
         channel_index < gltf_animation.channels.size(); ++channel_index) {
      ...
      if (gltf_channel.target_path == "translation") {
        ...
        LOGW("Gltf animation channel #{} has unsupported target path: {}",
             channel_index, gltf_channel.target_path);
        continue;
      } else {
        LOGW("Gltf animation channel #{} has unknown target path",
             channel_index);
        continue;
      }
      ...
      animation->add_channel(*nodes[gltf_channel.target_node], target,
                             samplers[gltf_channel.sampler]);
    }
  ...
  while (!traverse_nodes.empty()) {
    ...
    for (auto child_node_index : model.nodes[node_it.second].children) {
      traverse_nodes.push(
          std::make_pair(std::ref(current_node), child_node_index));
    }
  }
  ...
  if (!scene.has_component<sg::Light>()) {
    ...
    add_directional_light(
        scene, glm::quat({glm::radians(-90.0f), 0.0f, glm::radians(30.0f)}));
  }
