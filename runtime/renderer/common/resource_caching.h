...
struct hash<vkb::DescriptorSetLayout> {
  std::size_t operator()(
      const vkb::DescriptorSetLayout &descriptor_set_layout) const {
    std::size_t result = 0;
...
struct hash<vkb::Attachment> {
  std::size_t operator()(const vkb::Attachment &attachment) const {
    ...
    vkb::hash_combine(result, static_cast<std::underlying_type<VkFormat>::type>(
                                  attachment.format));
    vkb::hash_combine(
...
struct hash<vkb::SpecializationConstantState> {
  std::size_t operator()(const vkb::SpecializationConstantState
                             &specialization_constant_state) const {
    std::size_t result = 0;

    for (auto constants :
         specialization_constant_state.get_specialization_constant_state()) {
...
struct hash<vkb::ShaderResource> {
  std::size_t operator()(const vkb::ShaderResource &shader_resource) const {
    ...
    vkb::hash_combine(
        result,
        static_cast<std::underlying_type<vkb::ShaderResourceType>::type>(
            shader_resource.type));
...
struct hash<vkb::ColorBlendAttachmentState> {
  std::size_t operator()(
      const vkb::ColorBlendAttachmentState &color_blend_attachment) const {
    std::size_t result = 0;
...
struct hash<vkb::PipelineState> {
  std::size_t operator()(const vkb::PipelineState &pipeline_state) const {
    ...
    vkb::hash_combine(result,
                      pipeline_state.get_specialization_constant_state());
    ...
    vkb::hash_combine(
        result, pipeline_state.get_rasterization_state().depth_bias_enable);
    vkb::hash_combine(
        result, pipeline_state.get_rasterization_state().depth_clamp_enable);
    vkb::hash_combine(result,
    ...
    vkb::hash_combine(
        result, pipeline_state.get_multisample_state().alpha_to_one_enable);
    vkb::hash_combine(
        result, pipeline_state.get_multisample_state().min_sample_shading);
    vkb::hash_combine(
        result,
        static_cast<std::underlying_type<VkSampleCountFlagBits>::type>(
            pipeline_state.get_multisample_state().rasterization_samples));
    vkb::hash_combine(
        result, pipeline_state.get_multisample_state().sample_shading_enable);
    vkb::hash_combine(result,
    ...
    vkb::hash_combine(
        result, pipeline_state.get_depth_stencil_state().depth_test_enable);
    vkb::hash_combine(
        result, pipeline_state.get_depth_stencil_state().depth_write_enable);
    vkb::hash_combine(result, pipeline_state.get_depth_stencil_state().front);
    vkb::hash_combine(
        result, pipeline_state.get_depth_stencil_state().stencil_test_enable);
    ...
    for (auto &attachment :
         pipeline_state.get_color_blend_state().attachments) {
...
struct RecordHelper {
  ...
  void index(ResourceRecord & /*recorder*/, size_t /*index*/,
             T & /*resource*/) {}
};
...
T &request_resource(Device &device, ResourceRecord *recorder,
  ...
  LOGD("Building #%llu cache object (%s)\n", res_id, res_type);
...
// Only error handle in release
  ...
  try {
    ...
    LOGE("Creation error for #{%llu} cache object ({%s})\n", res_id, res_type);
    throw e;
  }
#endif
