...
#define DEFAULT_FENCE_TIMEOUT \
  100000000000  // Default fence timeout in nanoseconds
...
/**
 * @brief Helper function to determine a suitable supported depth format based
 *on a priority list
 * @param physical_device The physical device to check the depth formats against
 * @param depth_only (Optional) Wether to include the stencil component in the
 *format or not
 * @param depth_format_priority_list (Optional) The list of depth formats to
 *prefer over one another By default we start with the highest precision packed
 *format
...
/// @brief Helper function to determine if a Vulkan descriptor type is a dynamic
/// storage buffer or dynamic uniform buffer.
...
/// @brief Helper function to determine if a Vulkan descriptor type is a buffer
/// (either uniform or storage buffer, dynamic or not).
/// @param descriptor_type Vulkan descriptor type to check.
...
/// @brief Image memory barrier structure used to define memory access for an
/// image view during command recording.
 ...
 * explicitly give transition parameters.
 ...
 * @param subresource_range The VkImageSubresourceRange to use with the
 * transition.
 */
...
/**
 * @brief Put an image memory barrier for a layout transition of an image, on a
 * given subresource range.
 *
 * The src_stage_mask, dst_stage_mask, src_access_mask, and dst_access_mask used
 * are determined from old_layout and new_layout.
 *
 * @param command_buffer The VkCommandBuffer to record the barrier.
 * @param image The VkImage to transition.
 * @param old_layout The VkImageLayout to transition from.
...
/**
 * @brief Put an image memory barrier for a layout transition of an image, on a
 * fixed subresource with first mip level and layer.
 *
 * The src_stage_mask, dst_stage_mask, src_access_mask, and dst_access_mask used
 * are determined from old_layout and new_layout.
 *
