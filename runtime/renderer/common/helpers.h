...
template <typename T, typename... Args>
inline void write(std::ostringstream &os, const T &first_arg,
                  const Args &...args) {
  write(os, first_arg);
  write(os, args...);
}

/// @brief Helper function to combine a given hash with a generated hash for the
/// input param.
template <class T>
...
/// @brief Helper function to convert a data type to string using output stream operator.
