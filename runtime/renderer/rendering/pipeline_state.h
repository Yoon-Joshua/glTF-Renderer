...
struct DepthStencilState {
  ...
  // Note: Using reversed depth-buffer for increased precision, so Greater depth
  // values are kept
...
/// Helper class to create specialization constants for a Vulkan pipeline. The
/// state tracks a pipeline globally, and not per shader. Two shaders using the
/// same constant_id will have the same data.
...
template <class T>
inline void SpecializationConstantState::set_constant(std::uint32_t constant_id,
                                                      const T &data) {
  set_constant(constant_id, to_bytes(static_cast<std::uint32_t>(data)));
}

template <>
inline void SpecializationConstantState::set_constant<bool>(
