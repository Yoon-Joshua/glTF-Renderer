...
struct alignas(16) Light {
  ...
  glm::vec2 info;  // (only used for spot lights) info.x represents light inner
                   // cone angle, info.y represents light outer cone angle
};
...
/// @brief This class defines an interface for subpasses where they need to
/// implement the draw function. It is used to construct a RenderPipeline
class Subpass {
 public:
  Subpass(RenderContext &render_context, ShaderSource &&vertex_shader,
  ...
  /// @brief Updates the render target attachments with the ones stored in this
  /// subpass This function is called by the RenderPipeline before beginning the
  /// render pass and before proceeding with a new subpass.
  ...
  /// @tparam A light structure that has 'directional_lights', 'point_lights'
  /// and 'spot_light' array fields defined.
  /// @param scene_lights All of the light components from the scene graph
  /// @param light_count The maximum amount of lights allowed for any given type
  /// of light.
  template <typename T>
  void allocate_lights(const std::vector<sg::Light *> &scene_lights,
                       size_t light_count) {
    assert(scene_lights.size() <= (light_count * sg::LightType::Max) &&
           "Exceeding Max Light Capacity");
    ...
    for (auto &scene_light : scene_lights) {
      ...
      Light light{
          {transform.get_translation(),
           static_cast<float>(scene_light->get_light_type())},
    ...
    std::copy(lighting_state.directional_lights.begin(),
              lighting_state.directional_lights.end(),
              light_info.directional_lights);
    std::copy(lighting_state.point_lights.begin(),
              lighting_state.point_lights.end(), light_info.point_lights);
    std::copy(lighting_state.spot_lights.begin(),
              lighting_state.spot_lights.end(), light_info.spot_lights);

    auto &render_frame = get_render_context().get_active_frame();
    lighting_state.light_buffer = render_frame.allocate_buffer(
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(T));
 ...
 protected:
  ...
  /// The structure containing all the requested render-ready lights for the
  /// scene
 ...
 private:
  ...
  /// @brief When creating the renderpass, pDepthStencilAttachment will be set
  /// to nullptr, which disables depth testing
  bool disable_depth_stencil_attachment{false};

  /// @brief When creating the renderpass, if not None, the resolve of the
  /// multisampled depth attachment will be enabled, with this mode, to
