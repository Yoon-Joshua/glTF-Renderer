...
RenderContext::RenderContext(
  ...
  if (surface != VK_NULL_HANDLE) {
    ...
    VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        device.get_gpu().get_handle(), surface, &surface_properties));

    if (surface_properties.currentExtent.width == 0xFFFFFFFF) {
      swapchain = std::make_unique<Swapchain>(
          device, surface, present_mode, present_mode_priority_list,
  ...
  if (swapchain) {
    ...
    for (auto& image_handle : swapchain->get_images()) {
      auto swapchain_image =
          core::Image(device, image_handle, extent, swapchain->get_format(),
                      swapchain->get_usage());
      auto render_target =
          create_render_target_func(std::move(swapchain_image));
      frames.emplace_back(std::make_unique<RenderFrame>(
    ...
    auto color_image = core::Image(
        device, VkExtent3D{surface_extent.width, surface_extent.height, 1},
...
void RenderContext::update_swapchain(
    const VkExtent2D& extent, const VkSurfaceTransformFlagBitsKHR transform) {
  if (!swapchain) {
    LOGW(
        "Can't update the swapchains extent and surface transform in headless "
        "mode, skipping.");
    return;
  }
  ...
  if (transform == VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR ||
    ...
    // Pre-rotation: always use native orientation i.e. if rotated, use width
    // and height of identity transform
    std::swap(width, height);
  }

  swapchain = std::make_unique<Swapchain>(*swapchain, VkExtent2D{width, height},
                                          transform);
...
bool RenderContext::has_swapchain() { return swapchain != nullptr; }
...
void RenderContext::recreate() {
  ...
  for (auto& image_handle : swapchain->get_images()) {
    core::Image swapchain_image{device, image_handle, extent,
    ...
    if (frame_it != frames.end()) {
      ...
      // Create a new frame if the new swapchain has more images than current
      // frames
      frames.emplace_back(std::make_unique<RenderFrame>(
          device, std::move(render_target), thread_count));
    }
...
CommandBuffer& RenderContext::begin(CommandBuffer::ResetMode reset_mode) {
  assert(prepared &&
         "RenderContext not prepared for rendering, call prepare()");
...
void RenderContext::submit(const std::vector<CommandBuffer*>& command_buffers) {
  assert(frame_active &&
         "RenderContext is inactive, cannot submit command buffer. Please call "
         "begin()");

  VkSemaphore render_semaphore = VK_NULL_HANDLE;

  if (swapchain) {
    assert(acquired_semaphore &&
           "We do not have acquired_semaphore, it was probably consumed?\n");
...
void RenderContext::begin_frame() {
  ...
  if (swapchain) {
    auto result = swapchain->acquire_next_image(
        active_frame_index, acquired_semaphore, VK_NULL_HANDLE);

    if (result == VK_SUBOPTIMAL_KHR || result == VK_ERROR_OUT_OF_DATE_KHR) {
      bool swapchain_updated =
          handle_surface_changes(result == VK_ERROR_OUT_OF_DATE_KHR);

      if (swapchain_updated) {
        result = swapchain->acquire_next_image(
            active_frame_index, acquired_semaphore, VK_NULL_HANDLE);
      }
    }
...
void RenderContext::end_frame(VkSemaphore semaphore) {
  ...
  if (swapchain) {
    ...
    if (device.is_extension_supported(
        ...
        window.get_display_present_info(
            &disp_present_info, surface_extent.width, surface_extent.height)) {
...
Device& RenderContext::get_device() { return device; }
...
bool RenderContext::handle_surface_changes(bool force_update) {
  ...
  VkSurfaceCapabilitiesKHR surface_properties;
  VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      device.get_gpu().get_handle(), swapchain->get_surface(),
      &surface_properties));
