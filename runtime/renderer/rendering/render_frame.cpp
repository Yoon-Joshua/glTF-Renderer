...
#include "runtime/renderer/buffer_pool.h"
...
RenderFrame::RenderFrame(Device& device,
  ...
  for (auto& usage_it : supported_usage_map) {
    ...
    for (size_t i = 0; i < thread_count; ++i) {
      usage_buffer_pools.push_back(std::make_pair(
          BufferPool{device, BUFFER_POOL_BLOCK_SIZE * 1024 * usage_it.second,
                     usage_it.first},
          nullptr));
    }

    auto res_ins_it =
        buffer_pools.emplace(usage_it.first, std::move(usage_buffer_pools));
  ...
  for (size_t i = 0; i < thread_count; ++i) {
    descriptor_pools.push_back(
        std::make_unique<std::unordered_map<std::size_t, DescriptorPool>>());
    descriptor_sets.push_back(
        std::make_unique<std::unordered_map<std::size_t, DescriptorSet>>());
  }
}
...
void RenderFrame::reset() {
  ...
  if (descriptor_management_strategy ==
      vkb::DescriptorManagementStrategy::CreateDirectly) {
    clear_descriptors();
  }
}

Device& RenderFrame::get_device() { return device; }

const FencePool& RenderFrame::get_fence_pool() const { return fence_pool; }

VkFence RenderFrame::request_fence() { return fence_pool.request_fence(); }
...
void RenderFrame::update_render_target(
    std::unique_ptr<RenderTarget>&& render_target) {
  swapchain_render_target = std::move(render_target);
}
...
CommandBuffer& RenderFrame::request_command_buffer(
    const Queue& queue, CommandBuffer::ResetMode reset_mode,
  ...
  auto command_pool_it =
      std::find_if(command_pools.begin(), command_pools.end(),
...
VkDescriptorSet RenderFrame::request_descriptor_set(
  ...
  auto& descriptor_pool = request_resource(
      device, nullptr, *descriptor_pools[thread_index], descriptor_set_layout);
  if (descriptor_management_strategy ==
      DescriptorManagementStrategy::StoreInCache) {
    // The bindings we want to update before binding, if empty we update all
    // bindings
    std::vector<uint32_t> bindings_to_update;
    // If update after bind is enabled, we store the binding index of each
    // binding that need to be updated before being bound
    if (update_after_bind) {
      bindings_to_update = collect_bindings_to_update(
          descriptor_set_layout, buffer_infos, image_infos);
    }

    // Request a descriptor set from the render frame, and write the buffer
    // infos and image infos of all the specified bindings
    assert(thread_index < descriptor_sets.size());
    auto& descriptor_set = request_resource(
        device, nullptr, *descriptor_sets[thread_index], descriptor_set_layout,
        descriptor_pool, buffer_infos, image_infos);
    descriptor_set.update(bindings_to_update);
    return descriptor_set.get_handle();
  } else {
    // Request a descriptor pool, allocate a descriptor set, write buffer and
    // image data to it
    DescriptorSet descriptor_set{device, descriptor_set_layout, descriptor_pool,
...
BufferAllocation RenderFrame::allocate_buffer(const VkBufferUsageFlags usage,
  ...
  if (buffer_pool_it == buffer_pools.end()) {
    LOGE("No buffer pool for buffer usage {}\n", usage);
  ...
  bool want_minimal_block = buffer_allocation_strategy ==
                            BufferAllocationStrategy::OneAllocationPerBuffer;

  if (want_minimal_block || !buffer_block ||
      !buffer_block->can_allocate(size)) {
    // If we are creating a buffer for each allocation of there is no block
    // associated with the pool or the current block is too small for this
    // allocation, request a new buffer block
...
std::vector<std::unique_ptr<CommandPool>>& RenderFrame::get_command_pools(
    const Queue& queue, CommandBuffer::ResetMode reset_mode) {
  ...
  for (size_t i = 0; i < thread_count; i++) {
    queue_command_pools.push_back(std::make_unique<CommandPool>(
        device, queue.get_family_index(), this, i, reset_mode));
  }

  auto res_ins_it = command_pools.emplace(queue.get_family_index(),
                                          std::move(queue_command_pools));
...
std::vector<uint32_t> RenderFrame::collect_bindings_to_update(
  ...
  auto aggregate_binding_to_update =
      [&bindings_to_update, &descriptor_set_layout](const auto& infos_map) {
        for (const auto& pair : infos_map) {
          ...
          if (!(descriptor_set_layout.get_layout_binding_flag(binding_index) &
              ...
              std::find(bindings_to_update.begin(), bindings_to_update.end(),
                        binding_index) == bindings_to_update.end()) {
