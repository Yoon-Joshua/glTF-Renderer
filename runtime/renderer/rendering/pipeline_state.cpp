...
bool operator==(const vkb::ColorBlendAttachmentState &lhs,
                const vkb::ColorBlendAttachmentState &rhs) {
  return std::tie(lhs.alpha_blend_op, lhs.blend_enable, lhs.color_blend_op,
                  lhs.color_write_mask, lhs.dst_alpha_blend_factor,
                  lhs.dst_color_blend_factor, lhs.src_alpha_blend_factor,
                  lhs.src_color_blend_factor) ==
         std::tie(rhs.alpha_blend_op, rhs.blend_enable, rhs.color_blend_op,
...
bool operator!=(const vkb::StencilOpState &lhs,
                const vkb::StencilOpState &rhs) {
  return std::tie(lhs.compare_op, lhs.depth_fail_op, lhs.fail_op,
                  lhs.pass_op) !=
         std::tie(rhs.compare_op, rhs.depth_fail_op, rhs.fail_op, rhs.pass_op);
}

bool operator!=(const vkb::VertexInputState &lhs,
                const vkb::VertexInputState &rhs) {
  return lhs.attributes != rhs.attributes || lhs.bindings != rhs.bindings;
}

// bool operator!=(const vkb::InputAssemblyState &lhs,
//                 const vkb::InputAssemblyState &rhs) {
//   return std::tie(lhs.primitive_restart_enable, lhs.topology) !=
//          std::tie(rhs.primitive_restart_enable, rhs.topology);
// }

bool operator!=(const vkb::RasterizationState &lhs,
                const vkb::RasterizationState &rhs) {
  return std::tie(lhs.cull_mode, lhs.depth_bias_enable, lhs.depth_clamp_enable,
                  lhs.front_face, lhs.front_face, lhs.polygon_mode,
                  lhs.rasterizer_discard_enable) !=
         std::tie(rhs.cull_mode, rhs.depth_bias_enable, rhs.depth_clamp_enable,
                  rhs.front_face, rhs.front_face, rhs.polygon_mode,
...
bool operator!=(const vkb::MultisampleState &lhs,
                const vkb::MultisampleState &rhs) {
  return std::tie(lhs.alpha_to_coverage_enable, lhs.alpha_to_one_enable,
                  lhs.min_sample_shading, lhs.rasterization_samples,
                  lhs.sample_mask, lhs.sample_shading_enable) !=
         std::tie(rhs.alpha_to_coverage_enable, rhs.alpha_to_one_enable,
                  rhs.min_sample_shading, rhs.rasterization_samples,
                  rhs.sample_mask, rhs.sample_shading_enable);
}

bool operator!=(const vkb::DepthStencilState &lhs,
                const vkb::DepthStencilState &rhs) {
  return std::tie(lhs.depth_bounds_test_enable, lhs.depth_compare_op,
                  lhs.depth_test_enable, lhs.depth_write_enable,
                  lhs.stencil_test_enable) !=
             std::tie(rhs.depth_bounds_test_enable, rhs.depth_compare_op,
                      rhs.depth_test_enable, rhs.depth_write_enable,
                      rhs.stencil_test_enable) ||
         lhs.back != rhs.back || lhs.front != rhs.front;
}

bool operator!=(const vkb::ColorBlendState &lhs,
                const vkb::ColorBlendState &rhs) {
  return std::tie(lhs.logic_op, lhs.logic_op_enable) !=
             std::tie(rhs.logic_op, rhs.logic_op_enable) ||
         lhs.attachments.size() != rhs.attachments.size() ||
         !std::equal(lhs.attachments.begin(), lhs.attachments.end(),
                     rhs.attachments.begin(),
                     [](const vkb::ColorBlendAttachmentState &lhs,
                        const vkb::ColorBlendAttachmentState &rhs) {
                       return lhs == rhs;
                     });
}
...
bool SpecializationConstantState::is_dirty() const { return dirty; }

void SpecializationConstantState::clear_dirty() { dirty = false; }

void SpecializationConstantState::set_constant(
    uint32_t constant_id, const std::vector<uint8_t> &value) {
...
void PipelineState::set_specialization_constant(
...
void PipelineState::set_vertex_input_state(
    const VertexInputState &new_vertex_input_state) {
...
void PipelineState::set_rasterization_state(
    const RasterizationState &new_rasterization_state) {
...
void PipelineState::set_multisample_state(
    const MultisampleState &new_multisample_state) {
...
void PipelineState::set_depth_stencil_state(
    const DepthStencilState &new_depth_stencil_state) {
...
void PipelineState::set_color_blend_state(
    const ColorBlendState &new_color_blend_state) {
...
const RenderPass *PipelineState::get_render_pass() const { return render_pass; }

const SpecializationConstantState &
PipelineState::get_specialization_constant_state() const {
  return specialization_constant_state;
}
...
uint32_t PipelineState::get_subpass_index() const { return subpass_index; }
