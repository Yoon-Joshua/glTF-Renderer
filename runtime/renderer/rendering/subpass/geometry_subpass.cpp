...
GeometrySubpass::GeometrySubpass(RenderContext &render_context,
                                 ...
                                 sg::Scene &scene_, sg::Camera &camera)
    : Subpass{render_context, std::move(vertex_source),
...
void GeometrySubpass::draw(CommandBuffer &command_buffer) {
  ...
  // Draw opaque objects in front-to-back order
  {
    ...
    for (auto node_it = opaque_nodes.begin(); node_it != opaque_nodes.end();
         node_it++) {
      ...
      VkFrontFace front_face =
          flipped ? VK_FRONT_FACE_CLOCKWISE : VK_FRONT_FACE_COUNTER_CLOCKWISE;
  ...
  {
    ScopedDebugLabel transparent_debug_label{command_buffer,
                                             "Transparent objects"};

    for (auto node_it = transparent_nodes.rbegin();
         node_it != transparent_nodes.rend(); node_it++) {
      update_uniform(command_buffer, *node_it->second.first, thread_index);
...
void GeometrySubpass::set_thread_index(uint32_t index) { thread_index = index; }

void GeometrySubpass::update_uniform(CommandBuffer &command_buffer,
                                     sg::Node &node, size_t thread_index) {
  GlobalUniform global_uniform;

  global_uniform.camera_view_proj =
      camera.get_pre_rotation() *
  ...
  auto allocation = render_frame.allocate_buffer(
      VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(GlobalUniform), thread_index);

  global_uniform.model = transform.get_world_matrix();

  global_uniform.camera_position =
      glm::vec3(glm::inverse(camera.get_view())[3]);

  allocation.update(global_uniform);

  command_buffer.bind_buffer(allocation.get_buffer(), allocation.get_offset(),
                             allocation.get_size(), 0, 1, 0);
}

void GeometrySubpass::draw_submesh(CommandBuffer &command_buffer,
                                   sg::SubMesh &sub_mesh,
                                   VkFrontFace front_face) {
  auto &device = command_buffer.get_device();

  ScopedDebugLabel submesh_debug_label{command_buffer,
                                       sub_mesh.get_name().c_str()};

  prepare_pipeline_state(command_buffer, front_face,
                         sub_mesh.get_material()->double_sided);
  ...
  auto &pipeline_layout =
      prepare_pipeline_layout(command_buffer, shader_modules);

  command_buffer.bind_pipeline_layout(pipeline_layout);

  if (pipeline_layout.get_push_constant_range_stage(
          sizeof(PBRMaterialUniform)) != 0) {
    prepare_push_constants(command_buffer, sub_mesh);
  }

  DescriptorSetLayout &descriptor_set_layout =
      pipeline_layout.get_descriptor_set_layout(0);

  for (auto &texture : sub_mesh.get_material()->textures) {
    if (auto layout_binding =
            descriptor_set_layout.get_layout_binding(texture.first)) {
      command_buffer.bind_image(
          texture.second->get_image()->get_vk_image_view(),
          texture.second->get_sampler()->vk_sampler, 0, layout_binding->binding,
          0);
    }
  }

  auto vertex_input_resources = pipeline_layout.get_resources(
  ...
  for (auto &input_resource : vertex_input_resources) {
    ...
    if (buffer_iter != sub_mesh.vertex_buffers.end()) {
      ...
      command_buffer.bind_vertex_buffers(input_resource.location,
                                         std::move(buffers), {0});
    }
  }
...
void GeometrySubpass::prepare_pipeline_state(CommandBuffer &command_buffer,
                                             VkFrontFace front_face,
...
PipelineLayout &GeometrySubpass::prepare_pipeline_layout(
    CommandBuffer &command_buffer,
    const std::vector<ShaderModule *> &shader_modules) {
  // Sets any specified resource modes
  for (auto &shader_module : shader_modules) {
    for (auto &resource_mode : resource_mode_map) {
      shader_module->set_resource_mode(resource_mode.first,
                                       resource_mode.second);
    }
  }

  return command_buffer.get_device()
      .get_resource_cache()
      .request_pipeline_layout(shader_modules);
}

void GeometrySubpass::prepare_push_constants(CommandBuffer &command_buffer,
                                             sg::SubMesh &sub_mesh) {
  auto pbr_material =
      dynamic_cast<const sg::PBRMaterial *>(sub_mesh.get_material());
...
void GeometrySubpass::draw_submesh_command(CommandBuffer &command_buffer,
                                           sg::SubMesh &sub_mesh) {
  // Draw submesh indexed if indices exists
  if (sub_mesh.vertex_indices != 0) {
    // Bind index buffer of submesh
    command_buffer.bind_index_buffer(
        *sub_mesh.index_buffer, sub_mesh.index_offset, sub_mesh.index_type);
...
void GeometrySubpass::get_sorted_nodes(
    std::multimap<float, std::pair<sg::Node *, sg::SubMesh *>> &opaque_nodes,
