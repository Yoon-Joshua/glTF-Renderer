...
RenderTarget::RenderTarget(std::vector<core::Image>&& images)
  ...
  std::transform(this->images.begin(), this->images.end(),
                 std::inserter(unique_extent, unique_extent.end()),
                 get_image_extent);
  ...
  for (auto& image : this->images) {
    if (image.get_type() != VK_IMAGE_TYPE_2D) {
      throw VulkanException{VK_ERROR_INITIALIZATION_FAILED,
                            "Image type is not 2D"};
    }
    views.emplace_back(image, VK_IMAGE_VIEW_TYPE_2D);

    attachments.emplace_back(Attachment{
        image.get_format(), image.get_sample_count(), image.get_usage()});
  }
}

vkb::RenderTarget::RenderTarget(std::vector<core::ImageView>&& image_views)
    : device{const_cast<core::Image&>(image_views.back().get_image())
                 .get_device()},
      images{},
  ...
  auto get_view_extent = [](const core::ImageView& view) {
    ...
    return VkExtent2D{mip0_extent.width >> mip_level,
                      mip0_extent.height >> mip_level};
  };
...
const VkExtent2D& RenderTarget::get_extent() const { return extent; }
