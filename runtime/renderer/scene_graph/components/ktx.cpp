...
//     int mip_level, int face, int width, int height, int depth,
//     ktx_uint64_t face_lod_size, void *pixels, void *user_data) {
//   auto *callback_data = reinterpret_cast<CallbackData *>(user_data);
//   assert(static_cast<size_t>(mip_level) < callback_data->mipmaps->size() &&
//          "Not enough space in the mipmap vector");

//   ktx_size_t mipmap_offset = 0;
//   auto result = ktxTexture_GetImageOffset(callback_data->texture, mip_level, 0,
//                                           face, &mipmap_offset);
...
//       data_buffer, data_size, KTX_TEXTURE_CREATE_NO_FLAGS, &texture);
...
//         ktxTexture_LoadImageData(texture, mut_data.data(), size);
...
//     // in sRGB, but the format we get back won't report that, so this will
...
//   auto result = ktxTexture_IterateLevels(texture, optimal_tiling_callback,
//                                          &callback_data);
//   if (result != KTX_SUCCESS) {
//     throw std::runtime_error("Error loading KTX texture");
//   }

//   // If the texture contains more than one layer, then populate the offsets
//   // otherwise take the mipmap level offsets
...
//       offsets[0].push_back(
//           static_cast<VkDeviceSize>(mipmap_levels[level].offset));
//     }
//     set_offsets(offsets);
//   }
