...
#include "runtime/renderer/scene_graph/components/stb.h"
#include "runtime/renderer/scene_graph/components/ktx.h"
namespace sg {
...
// may want to assume that the loaded data is in sRGB format (since it usually
// is). In those cases, this helper will get called which will force an existing
// unorm format to become an srgb format where one exists. If none exist, the
// format will remain unmodified.
...
Image::Image(const std::string &name, std::vector<uint8_t> &&d,
             std::vector<Mipmap> &&m)
    : Component{name},
      data{std::move(d)},
...
std::type_index Image::get_type() { return typeid(Image); }
...
const uint32_t Image::get_layers() const { return layers; }

const std::vector<Mipmap> &Image::get_mipmaps() const { return mipmaps; }

const std::vector<std::vector<VkDeviceSize>> &Image::get_offsets() const {
  return offsets;
}

void Image::create_vk_image(vkb::Device const &device,
                            VkImageViewType image_view_type,
                            VkImageCreateFlags flags) {
  assert(!vk_image && !vk_image_view && "Vulkan image already constructed");

  vk_image = std::make_unique<vkb::core::Image>(
      device, get_extent(), format,
      VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
      VMA_MEMORY_USAGE_GPU_ONLY, VK_SAMPLE_COUNT_1_BIT,
      vkb::to_u32(mipmaps.size()), layers, VK_IMAGE_TILING_OPTIMAL, flags);
  vk_image->set_debug_name(get_name());

  vk_image_view =
      std::make_unique<vkb::core::ImageView>(*vk_image, image_view_type);
  vk_image_view->set_debug_name("View on " + get_name());
}

const vkb::core::Image &Image::get_vk_image() const {
  assert(vk_image && "Vulkan image was not created");
  return *vk_image;
}

const vkb::core::ImageView &Image::get_vk_image_view() const {
...
void Image::generate_mipmaps() {
  ...
  while (true) {
    ...
    stbir_resize_uint8(
        data.data() + prev_mipmap.offset, prev_mipmap.extent.width,
        prev_mipmap.extent.height, 0, data.data() + next_mipmap.offset,
...
std::vector<Mipmap> &Image::get_mut_mipmaps() { return mipmaps; }

std::vector<uint8_t> &Image::get_mut_data() { return data; }
...
void Image::set_format(const VkFormat f) { format = f; }
...
void Image::set_layers(uint32_t l) { layers = l; }

void Image::set_offsets(const std::vector<std::vector<VkDeviceSize>> &o) {
  offsets = o;
}

void Image::coerce_format_to_srgb() { format = maybe_coerce_to_srgb(format); }

#define TEMP
std::unique_ptr<Image> Image::load(const std::string &name,
