...
#include "runtime/renderer/scene_graph/components/transform.h"
...
void FreeCamera::update(float delta_time) {
  ...
  if (delta_rotation != glm::vec3(0.0f, 0.0f, 0.0f) ||
      delta_translation != glm::vec3(0.0f, 0.0f, 0.0f)) {
    auto &transform = get_node().get_component<Transform>();

    glm::quat qx =
        glm::angleAxis(delta_rotation.x, glm::vec3(1.0f, 0.0f, 0.0f));
    glm::quat qy =
        glm::angleAxis(delta_rotation.y, glm::vec3(0.0f, 1.0f, 0.0f));
...
void FreeCamera::input_event(const InputEvent &input_event) {
  if (input_event.get_source() == EventSource::Keyboard) {
    ...
    glm::vec2 mouse_pos{std::floor(mouse_button.get_pos_x()),
                        std::floor(mouse_button.get_pos_y())};
    ...
    glm::vec2 touch_pos{std::floor(touch_event.get_pos_x()),
                        std::floor(touch_event.get_pos_y())};
    ...
    if (touch_event.get_action() == TouchAction::Move &&
        touch_event.get_pointer_id() == 0) {
...
void FreeCamera::resize(uint32_t width, uint32_t height) {
  ...
  if (camera_node.has_component<Camera>()) {
    if (auto camera = dynamic_cast<PerspectiveCamera *>(
            &camera_node.get_component<Camera>())) {
