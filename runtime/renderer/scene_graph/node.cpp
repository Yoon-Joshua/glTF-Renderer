...
Node::Node(const size_t id, const std::string &name)
    : id{id}, name{name}, transform{*this} {
  set_component(transform);
}

const size_t Node::get_id() const { return id; }

const std::string &Node::get_name() const { return name; }
...
const std::vector<Node *> &Node::get_children() const { return children; }
