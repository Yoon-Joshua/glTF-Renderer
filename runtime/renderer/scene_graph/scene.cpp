...
void Scene::set_name(const std::string &new_name) { name = new_name; }
...
void Scene::add_child(Node &child) { root->add_child(child); }
...
void Scene::set_root_node(Node &node) { root = &node; }

Node &Scene::get_root_node() { return *root; }
