...
BufferAllocation::BufferAllocation(core::Buffer& buffer, VkDeviceSize size,
                                   VkDeviceSize offset)
    : buffer{&buffer}, size{size}, base_offset{offset} {}

void BufferAllocation::update(const std::vector<uint8_t>& data,
                              uint32_t offset) {
...
bool BufferAllocation::empty() const { return size == 0 || buffer == nullptr; }

VkDeviceSize BufferAllocation::get_size() const { return size; }

VkDeviceSize BufferAllocation::get_offset() const { return base_offset; }
...
BufferBlock::BufferBlock(Device& device, VkDeviceSize size,
  ...
  if (usage == VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT) {
    ...
    alignment =
        device.get_gpu().get_properties().limits.minTexelBufferOffsetAlignment;
  } else if (usage == VK_BUFFER_USAGE_INDEX_BUFFER_BIT ||
             usage == VK_BUFFER_USAGE_VERTEX_BUFFER_BIT ||
             usage == VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT) {
    // Used to calculate the offset, required when allocating memory (its value
...
VkDeviceSize BufferBlock::get_size() const { return buffer.get_size(); }

void BufferBlock::reset() { offset = 0; }

VkDeviceSize BufferBlock::aligned_offset() const {
  return (offset + alignment - 1) & ~(alignment - 1);
}

BufferPool::BufferPool(Device& device, VkDeviceSize block_size,
                       VkBufferUsageFlags usage, VmaMemoryUsage memory_usage)
    : device{device},
...
BufferBlock& BufferPool::request_buffer_block(const VkDeviceSize minimum_size,
  ...
  if (it == buffer_blocks.end()) {
    LOGD("Building #%lld buffer block ({})\n", buffer_blocks.size(), usage);

    VkDeviceSize new_block_size =
        minimal ? minimum_size : std::max(block_size, minimum_size);

    // Create a new block and get the iterator on it
    it = buffer_blocks.emplace(
        buffer_blocks.end(), std::make_unique<BufferBlock>(
                                 device, new_block_size, usage, memory_usage));
  }
